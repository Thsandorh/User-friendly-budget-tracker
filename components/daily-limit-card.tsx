...
import { CheckCircle2, XCircle, TrendingDown, Edit, Save, X } from "lucide-react"
...
export function DailyLimitCard() {
  ...
  const fetchDailyLimit = async () => {
    try {
      ...
      const response = await fetch(`/api/daily-limits?date=${today}`)
    ...
    try {
      ...
      if (response.ok) {
        toast({
          ...
          description: "Napi limit beállítva!",
        })
        setIsEditing(false)
        fetchDailyLimit()
      } else {
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Napi költségvetési limit</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  if (!data?.limit && !isEditing) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Napi költségvetési limit</CardTitle>
          <CardDescription>Állíts be napi költési limitet</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card className={exceeded ? "border-red-500 border-2" : "border-green-500 border-2"}>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        {isEditing ? (
          <div className="space-y-4">
            <div>
              <Label htmlFor="limit-amount">Napi limit (Ft)</Label>
              <Input
                id="limit-amount"
                type="number"
                value={limitAmount}
          ...
          <>
            ...
            <div className={`p-4 rounded-lg ${
              exceeded ? 'bg-red-50 dark:bg-red-950/20' : 'bg-green-50 dark:bg-green-950/20'
            }`}>
              {exceeded ? (
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-red-600 dark:text-red-500">
                    {Math.abs(remaining).toLocaleString('hu-HU')} Ft-tal többet költöttél
                  </p>
                </div>
              ) : (
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-green-600 dark:text-green-500">
                    Még {remaining.toLocaleString('hu-HU')} Ft maradt ma
                  </p>
                </div>
              )}
            </div>
